...
@Component({
  selector: 'app-finance-payable',
  templateUrl: './finance-payable.component.html',
  styleUrls: ['./finance-payable.component.css'],
  providers: [ChangeDateFormatService, DatatableService, TranslateService, ToastrService]
})

export class FinancePayableComponent implements OnInit {
  ...
  rangeEndDate: any;
  userGroupData: any;
  lastDayDate: { date: { year: number; month: number; day: number; }; };
  ...
  data = [];
  error1: any;
  today = this.changeDateFormatService.getToday();
  ...
  constructor(
    ...
    this.lastDayDate = this.changeDateFormatService.getWeekToday();
    $(document).on('click', '#ListTable .listTableCheck', function (event) {
      ...
      if ($(this).prop("checked") == true) {
        ...
        if (rowData.recInd == 'F') {
          ...
          let i = self.payableSumKeyArray.indexOf(rowData.referenceCd);
          ...
          if (rowData.payeeType == "Cardholder") {
            ...
            if (index !== -1) self.payeeTansectionArray.splice(index, 1);
  ...
  runRecordRecive(e) {
    if (this.reciveClaim.length == 0) {
      this.ToastrService.error("No Records Selected ")
    }
    ...
    this.hmsDataService.postApi(url, paramData).subscribe(data => {
      if (data.code == 200 && data.status == "OK") {
        this.filterSearch()
        this.ToastrService.success("Record Receipt Generated");
      } else {
  ...
  */
  ...
  broker(event = null) {
    ...
    if (this.paymentGenerateForm.valid) {
      ...
      this.ToastrService.error("Please Select Corporate Type");
  ...
  step4GeneratingChequeCo() {
    ...
    this.printCheque() // print company cheque
    return promise;
  }
  ...
  attachedClaimStatusChange(event) {
    ...
    this.diciplinePopupArr = ["D", "V", "DR", "H", "HS"]
  ...
  ngOnInit() {
    ...
    for (let i = 0; i < tempTransTypes.length; i++) {
      this.tarnsTypes.push({ 'id': tempTransTypes[i], 'itemName': tempTransTypes[i] });
    };
  }
  ...
  onSelectFocus(e) {
    if (this.payableSumKeyArray.length <= 0 && this.companySumKeyArray.length <= 0 && this.brokerSumKeyArray.length <= 0) {
      ...
      this.ToastrService.error(this.translate.instant('Error. No Transection is Selected '))
      return false
    } else {
      this.transactionSearch.patchValue({
        'paymentType': "E"
  ...
  CompanyRefundSteps() {
    if (this.paymentGenerateForm.valid) {
      ...
      this.ToastrService.error("Please Select Coporate Type");
      this.validateAllFormFields(this.paymentGenerateForm)
    }
  }
  GeneratingCoEft() {
      ...
      this.errorClaimsList(this.paymentGenerateForm.value).then(res => {
        ...
        setTimeout(() => {
          if (this.paymentGenerateForm.value.payeetype == 'all' && this.paymentGenerateForm.value.planType == 'S') {
            ...
            if (this.paymentGenerateForm.value.planType == 'Q' && this.paymentGenerateForm.value.payeetype == 'all') {
              if (!this.brokerCalled) {
                this.brokerCalled = true;
                this.broker();
              }
            } else {
              this.getBrokersList();
  ...
  checkingParams(paramData) {
    let promise = new Promise((resolve, reject) => {
      ...
      this.paramData = Object.assign(this.paramData, {
        "peftDueDate": this.changeDateFormatService.convertDateObjectToString(this.today)
      });
      this.hmsDataService.postApi(url, this.paramData).subscribe(data => {
        if (data.code == 200 && data.status == "OK") {
          this.eftFileNo = data.result.eftFileNo
          ...
          this.eftDueDate = this.changeDateFormatService.convertDateObjectToString(this.today)
          this.apiResponse = 1;
          resolve();
  ...
  exportingEftFile(formData) {
    let promise = new Promise((resolve, reject) => {
      ...
      let param = {
        "payableSumKey": this.payableSumKey,
        "paymentSumKey": +this.paymentSumKey || this.lastSelectedPaymentSumKey,
        "eftSumDueDate": this.changeDateFormatService.convertDateObjectToString(this.today),
 ...
 */
  ...
  onChangepayeeBankType(bankData) {
    ...
    if (this.transactionSearch.valid) {
      this.exDialog.openConfirm('Are You Sure ,You Want To Run Issue Payment ?').subscribe((value) => {
        if (value) {
  ...
  getBrokersList() {
    this.reqParam = [{ key: 'trxnStartDate', value: this.changeDateFormatService.convertDateObjectToString(this.todayDate) },{ key: 'trxnEndDate', value: this.changeDateFormatService.convertDateObjectToString(this.lastDayDate) }
    ];
    this.columns = [
      ...
      { title: 'Reference Info', data: 'referenceInfo' }
    ]
    this.finalTotal = '$0.00';
    this.total = 0;
    var url = FinanceApi.payableTransactions;
  ...
  exportList() {
    let data = this.dataTableService.getFooterParamsSearchTable("ListTable", {
    })
    var paramApp = this.currentUserService.getApplicationNameByRoleKey(+this.currentUserService.applicationRoleKey);
    ...
    if (this.getLowerSearch == true) {
      ...
      params[3] = ({ "key": "ppayeeType", value: this.payeeTypeArray });
    ...
    this.Step3exportCoRefundEft().then(res => {
      if (this.apiResponse == 1) {
        ...
        let generatingCoEftRecordsArr = {
          ...
          "result": "<p><a href='" + this.exportPdfFilePathUrl + "'>" + this.exportPdfFilePathUrl + "</a></p><p><a href='" + this.companyRefundExportPdfFilePathUrl + "'>" + this.companyRefundExportPdfFilePathUrl + "</a></p>"
        }
        this.fieldArraySecond.push(generatingCoEftRecordsArr);
        if (this.brokerSumKeyArray.length > 0) {
          this.brokerSecondHalf(false);
  ...
  payableReports(){
   this.searchEligibilityFiles()
    this.hmsDataService.OpenCloseModal("reportListBtn");   
  }
