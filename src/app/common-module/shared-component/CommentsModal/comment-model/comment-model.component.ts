...
@Component({
  ...
  providers: [DatatableService, DatePipe,ChangeDateFormatService]
})
...
export class CommentModelComponent implements OnInit {
  ...
  constructor(
    ...
    cardService.resetComments.subscribe((value) => {
      this.comments = [];
      this.commentjson = [];
    ...
    cardService.updateComments.subscribe((value) => {
      ...
      value.cardHolderCommentDtos.forEach(element => {
        let obj = {
          ...
          username: username,
          commentTxt: element.chComTxt,
          expiredOn:element.expiredOn,
          commentImportance: (element.chComImportance == '' || element.chComImportance == null) ? "N" : "Y"
        };
        if (obj.commentImportance) {
