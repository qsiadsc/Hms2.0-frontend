...
@Component({
  selector: 'app-uft-continuty-call-in',
  templateUrl: './uft-continuty-call-in.component.html',
  styleUrls: ['../dashboard.component.css'],
  providers: [ChangeDateFormatService, DatatableService]
})
export class UftContinutyComponentCallin implements OnInit {
  ...
  ngOnInit() {
    ...
    this.removePayeeText = this.translate.instant('uft.dashboard.uft-continuity.removePayee')
    this.addPayeeText = this.translate.instant('uft.dashboard.uft-continuity.addPayee')
  }
      ...
      if (type == 'default') {
        ...
        if (new Date() <= new Date(effDate)) {
          ...
          if (new Date(effDate).getFullYear() < new Date().getFullYear() && (this.date_diff_indays(new Date(effDate), new Date()) > 365)) {
            this.dateRangeList = [];
            this.dateRangeList = [
              { dateRangeKey: 'Current Contract Year', dateRangeValue: "Current Contract Year" },
      ...
      if (this.transType == '93' || this.transType == '80' || this.transType == '81') {
        this.getCompanyActivebankAccount().then(res => {
          if (Object.getOwnPropertyNames(this.companyBankArray).length > 0) {
            ...
            //Code from here shifted to function openBankForm()
...
this.showData = val;
setTimeout(() => {
  $("#fgpc_procSearchFilterr").trigger("click")
  ...
  onCompanyNameSelected(selected: CompleterItem) {
    if (selected) {
      ...
      this.compKey = selected.originalObject.coKey
      this.coId = selected.originalObject.coId
      this.compName = selected.originalObject.coName
      this.companyFieldVal(this.selecteCoName+" / "+this.coId||"");  
      ...
      this.compEffDate = ''
      this.getPredictiveCompanySearchData2(this.completerService)
    $("#fgpc_procSearchFilter").trigger("click")
    }
  }
      ...
      switch (gridType) {
        case "dailyPAP":
          this.columns_dailyPAP = [
            { title: this.translate.instant('uft.dashboard.uft-continuity.compNameNo'), data: 'coKey' },
            { title: this.translate.instant('uft.dashboard.uft-continuity.transactionAmount'), data: 'unitFinancialTransAmt' },
            { title: this.translate.instant('uft.dashboard.uft-continuity.transactionDate'), data: 'unitFinancialTransDt' },
            { title: this.translate.instant('uft.dashboard.uft-continuity.transactionDescription'), data: 'unitFinancialTransDesc' },
            { title: this.translate.instant('uft.dashboard.uft-continuity.transactionReference'), data: 'unitFinancialTransRef' },
            { title: this.translate.instant('uft.dashboard.uft-continuity.currentBalance'), data: 'paymentSumKey' },
          ];
  ...
  */
  onFileChanged(event) {
    ...
    if (fileSize > 2097152) {
      this.error3 = { isError: true, errorMessage: this.translate.instant('common.fileSizeShuoldNotGreaterThan2Mb') };
    ...
    if (!this.allowedValue) {
      this.error2 = { isError: true, errorMessage: this.translate.instant('common.onlyPdfFileTypeAreAllowed') };
      this.showRemoveBtn = true;
    } else {
      this.error2 = { isError: false, errorMessage: '' };
      this.showRemoveBtn = true;
  ...
  */
  ...
  getDailyPapBatch() {
    ...
    this.hmsDataService.postApi(UftApi.generateDailyPapBatchUrl, reqParam).subscribe(data => {
      if (data.code == 200 && data.status == "OK") {
        this.showPageLoader = false
        this.generateDailyPapPathUrl = data.result.generatePapEftTextFilePath
        if (this.generateDailyPapPathUrl != "") {
          window.open(this.generateDailyPapPathUrl, '_blank');
        }
      } else if (data.code == 404 && data.status == "NOT_FOUND") {
        this.showPageLoader = false
        this.generateDailyPapPath = this.translate.instant('uft.toaster.fileNotGenerated')
        $('#dailyPapEftPdfPopupBtn').trigger('click')
