...
import { Component, OnInit, ViewChild, Output, EventEmitter, Renderer2, Input } from '@angular/core';
...
import { ClaimApi } from '../../../../app/claim-module/claim-api';
...
export class BankReconcilationComponent implements OnInit {
  ...
  myDatePickerOptionsTill = CommonDatePickerOptions.myDatePickerDisableFutureDateOptions;
  error: any;
  businessType;
  fundingTotal;
  paymentRunTotal;
  ...
  fileType = []
  manualFilterFormGroup: FormGroup;
  manualOutsideBtn: boolean = false
  showManualLoader: boolean = true;
  constructor(
    ...
    this.dataTableService.emitobj.subscribe((val) => {
      this.latestFileId = val.fileId
  ...
  ngOnInit() {
    ...
    $(document).on('blur', '.table_date', function (e) {
      let id = e.target.id;
      let indexArray = id.split('_');
      let index = indexArray[1];
      let data_id = '#select_' + index
      ...
      $(this).val(date);
      let vdata = $(data_id).data('id');
      let val = $(data_id).val();
      vdata.clearDate = date;
      vdata.index = index;
      vdata.txnStatus = val
      ...
      if (dat.length == 0 && val !== '') {
        self.changedArray.push(vdata);
      }
    })

    $(document).on('change', '.bankHistory', function (e) {
      let val = $(this).val();
      let vdata = $(this).data('id');
      let dataRow = vdata;
      let id = e.target.id;
      let indexArray = id.split('_');
      let index = indexArray[1];
      let data_id = '#date_' + index
      let date_val = $(data_id).val();
      vdata.clearDate = date_val;
      vdata.index = index;
      let dat = self.changedArray.filter(obj => {
        if (obj.index == index) {
          obj.txnStatus = val;
          obj.clearDate = date_val;
          return obj;
        }
      })
      if (dat.length == 0) {
      ...
      if (val == 'C') {
        var $row = $(this).closest("tr"),
          $tds = $row.find("td:nth-child(4)").text();
        var chnge = "<input class=' fm-txt form-control bankReconAcNo' id='bankReconAcNo_" + index + "' type='text' value= '" + $tds + "'></input>";
        $row.find("td:nth-child(4)").html(chnge);
        let tdr = $row.find("td:nth-child(5)").text();
        tdr = tdr.replace("$", '')
        var chnge = "<input class='fm-txt form-control bankReconAmt' id='bankReconAmt_" + index + "' type='text' value= '" + tdr + "'></input>";
        $row.find("td:nth-child(5)").html(chnge);
      } else {
        let AcNo
        let Amt
        let dat = self.changedArray.filter(obj => {
          if (obj.index == index) {
            AcNo = obj.oldChequeAcct || obj.chequeAcct;
            Amt = obj.oldtranAmt || obj.tranAmt;
            obj.chequeAcct = AcNo
            obj.tranAmt = Amt
            return obj;
          }
        ...
        var $row = $(this).closest("tr");
        $row.find("td:nth-child(4)").html(AcNo);
        $row.find("td:nth-child(5)").html('<span>$</span>' + Amt);
      }
    });

    $(document).on('blur', '.bankReconAcNo', function (e) {
      let id = e.target.id;
      let indexArray = id.split('_');
      let index = indexArray[1];
      let newAcNo = $(this).val().toString();
      var pattern = new RegExp('^[0-9]{1,10}$');
      let isAlphaNumeric = pattern.test(newAcNo);
      if (isAlphaNumeric) {
        $(this).removeClass("error_field")
        let dat = self.changedArray.filter(obj => {
          if (obj.index == index) {
            obj.oldChequeAcct = obj.chequeAcct;
            obj.chequeAcct = newAcNo;
            return obj;
          }
        })
      } else {
        $(this).addClass("error_field")
        self.toastrService.error(self.translate.instant('uft.toaster.pleaseEnterValidChequeNumber'))
      }
    })

    $(document).on('blur', '.bankReconAmt', function (e) {
      let id = e.target.id;
      let indexArray = id.split('_');
      let index = indexArray[1];
      let tranAmt = $(this).val().toString()
      tranAmt = tranAmt.replace("$", '');
      var valid_dollar_amt_regex = /^\$?[0-9][0-9,]*[0-9]\.?[0-9]{0,2}$/i;
      let isNumeric = valid_dollar_amt_regex.test(tranAmt);
      if (isNumeric) {
        $(this).removeClass("error_field")
        let dat = self.changedArray.filter(obj => {
          if (obj.index == index) {
            obj.oldtranAmt = obj.tranAmt;
            obj.tranAmt = tranAmt
            return obj;
          }
        })
        ...
        $(this).addClass("error_field")
        self.toastrService.error(self.translate.instant('uft.toaster.pleaseEnterValidAmount'))
      }
    })
    ...
    this.otpBankAccountsForm = new FormGroup({
      ...
      'account': new FormControl('', [Validators.required, Validators.minLength(7), Validators.maxLength(10), CustomValidators.alphaNumeric, CustomValidators.validBankNo, CustomValidators.notEmpty]),
    ...
    this.getlisting();
    // Log #1103: Manual Popup changes
    this.manualFilterFormGroup = new FormGroup({
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      this.hmsDataService.getApi(URL).subscribe(data => {
        ...
        for (let index = 0; index < arr.length; index++) {
          ...
          this.dateRangeList.push({ dateRangeKey: element.businessTypeCd, dateRangeValue: element.businessTypeDesc })
        }
  ...
  changeDateFormat(event, frmControlName, formName, currentDate = false) {
    if (event.reason == 2 && currentDate == true && (event.value == null || event.value == '')) {
      ...
      this.manualFilterFormGroup.patchValue(datePickerValue)
    ...
    if (event.reason == 2) {
      ...
      if(formName == 'manualFilterFormGroup'){
        if(datePickerValue != null)
        this.manualFilterFormGroup.patchValue(datePickerValue)
      }
    }
      ...
      this.hmsDataService.getApi(companyUrl).subscribe(data => {
        if (data.hmsMessage.messageShort == "RECORD_GET_SUCCESSFULLY") {
          ...
          this.addOTPform.patchValue({
            ...
            "companyNameNo": this.compKey//this.selectedNameNo,
          })
        }
      })
    } else {
  ...
  addOtpForm(type, tabType) {
    ...
    if (tabType == 'REFUND') {
      ...
      if (this.refPDS) {
        this.radioSelected('81')
        this.addOTPform.patchValue({
          'transactionType': '81'
        })
      } else {
        this.radioSelected('80')
        this.addOTPform.patchValue({
          'transactionType': '80'
        })
      }
  ...
  */
  showNewGrid(companyId = null, transType = null) {
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      if (!$.fn.dataTable.isDataTable('#unitFinancialTransactionListReportGrid')) {
        ...
        this.dataTableService.jqueryDataTable(tableId, url, 'full_numbers', this.columns_new, 5, true, true, 'lt', 'irp', undefined, [0, 'asc'], '', reqParam, tableActions, undefined, [1], '', null, null, '', [0], [1, 2, 3, 4, 5])
      } else {
  ...
  submitOTPForm() {
    if (this.addOTPform.valid) {
      if (this.transType == '93' || this.transType == '80' || this.transType == '81') {
    ...
    if (this.addOTPform.valid) {
      ...
      this.hmsDataService.postApi(url, this.otpDataRequest).subscribe(data => {
        if (data.code == 200 && data.status == "OK") {
          ...
          if (this.fundingTabType == 'FUNDING') {
  ...
  */
  reversePap() {
    if (this.unitFinancialTransKey) {
      if (this.adjustedFlag == 'T') {
        ...
        this.exDialog.openConfirm(this.translate.instant(this.translate.instant('uft.toaster.reverseAmtConfirm') + " '" + this.currentUserService.convertAmountToDecimalWithDoller(this.unitFinancialTransAmount) + "' " + this.translate.instant('uft.toaster.forCompany') + " '" + this.unitFinancialCompanyName + "' ")).subscribe((value) => {
          if (value) {
            let reversePapData = {
  ...
  getOpeningClosingBalance(iconClick=null) {
    if(!this.uftContinuityData.valid){
      this.validateAllFormFields(this.filterReverse)
      return false;
    }
    let startDate  = this.changeDateFormatService.convertDateObjectToString(this.uftContinuityData.value.fromDate)
    ...
    let submitData = {
      ...
      "businessTypeCd": this.bTypeSelected || ''
    }
    this.hmsDataService.postApi(UftApi.getBankReconciliationUrl, submitData).subscribe(data => {
      if (data.code == 200 && data.hmsMessage.messageShort === "RECORD_GET_SUCCESSFULLY") {
        ...
        let openingBalance = this.currentUserService.convertAmountToDecimalWithDoller(data.result.opUncChqAmt != '' ? Math.abs(data.result.opUncChqAmt) : 0);
        let closingBalance = this.currentUserService.convertAmountToDecimalWithDoller(data.result.clUncChqAmt != '' ? Math.abs(data.result.clUncChqAmt) : 0);
        let oldChqClrAmt = this.currentUserService.convertAmountToDecimalWithDoller(data.result.oldChqClrAmt != '' ? Math.abs(data.result.oldChqClrAmt) : 0);
        let newChqIssuedAmt = this.currentUserService.convertAmountToDecimalWithDoller(data.result.newChqIssuedAmt != '' ? Math.abs(data.result.newChqIssuedAmt) : 0);
        let chqAdjustAmt = this.currentUserService.convertAmountToDecimalWithDoller(data.result.chqAdjustAmt != '' ? Math.abs(data.result.chqAdjustAmt) : 0);
        this.openingBalance = openingBalance;
        this.closingBalance = closingBalance;
        this.oldChqClrAmt = oldChqClrAmt;
        this.newChqIssuedAmt = newChqIssuedAmt;
        this.chqAdjustAmt = chqAdjustAmt;
      } else {
        this.showOpeningClosingBalLoader = false;
        this.openingBalance = this.currentUserService.convertAmountToDecimalWithDoller(0);
        this.closingBalance = this.currentUserService.convertAmountToDecimalWithDoller(0);
        this.oldChqClrAmt = this.currentUserService.convertAmountToDecimalWithDoller(0);
        this.newChqIssuedAmt = this.currentUserService.convertAmountToDecimalWithDoller(0);
        this.chqAdjustAmt = this.currentUserService.convertAmountToDecimalWithDoller(0);
      }
    })
  }
    ...
    this.hmsDataService.postApi(infoUrl, coInfoKey).subscribe(data => {
      if (data.code == 200 && data.status == "OK") {
        ...
        this.addOTPform.patchValue({
          ...
          'yearEndBalance': (data.result.yearendbalance) != undefined ? (Number.isInteger(data.result.yearendbalance) ? data.result.yearendbalance + '.00' : this.currentUserService.convertAmountToDecimalWithoutDoller(data.result.yearendbalance)) : this.currentUserService.convertAmountToDecimalWithoutDoller(0),
  ...
  onSubmitReverse(reverseData) {
    ...
    if (this.filterReverse.valid) {
      ...
      if (this.transType == 80) {
        ...
        /* Patch Grid column filters */
        if (!$.fn.dataTable.isDataTable('#unitFinancialTransactionList_PDS')) {
          this.dataTableService.jqueryDataTable(tableId, url, 'full_numbers', this.columns, this.currentUserService.defaultRecordsGrid, true, true, 'lt', 'irp', undefined, [0, 'asc'], '', this.reverseReqParam, tableActions, undefined, [1, 5], '', [3], '', '', '', [1, 5])
        } else {
          this.dataTableService.jqueryDataTableReload(tableId, url, this.reverseReqParam)
        }
      }
  ...
  */
  onDateRangeSelected(selected: CompleterItem) {
    if (selected) {
      this.bTypeSelected = selected.originalObject.dateRangeKey;
      this.uftContinuityData.patchValue({ 'dateRange': this.bTypeSelected })
      // this.getOpeningClosingBalance()
    } else {
    }
  }
  ...
  patchDateRange(selectedDateRange) {
    switch (selectedDateRange) {
      case 'Current Contract Year': //Current Contract Year
        var contractYear = this.changeDateFormatService.getCurrentContractYear(this.compEffDateMainUft)
        this.uftContinuityData.patchValue({ 'fromDate': contractYear.fromDate, 'toDate': contractYear.toDate });
        break;
  ...
  onBlurDateRange(anyy) {
  }

  addPayee() {
    this.showAddPayee = !this.showAddPayee
  ...
  */
  ...
  getList(noFile = null) {
    ...
    var date = new Date();
    let firstDay
    this.changeDateFormatService.getToday().date.day <= 4 ? firstDay = firstDay = new Date(date.getFullYear(), date.getMonth() - 1, 1) : firstDay = new Date(date.getFullYear(), date.getMonth(), 1);
    let edate =  this.changeDateFormatService.getToday()
    this.data = [
    ];
    let tableId = 'bank-history';
    let pType = this.uftContinuityData.value.bType;
    let startDate = '';
    let endDate
    let bankDate = ''
    let fromDate = ''
    if (noFile == null) {
      // Log #1103: Manual Date Filter code commented
      this.manualOutsideBtn = true
      this.fileID = '';
      bankDate = this.uftContinuityData.value.toDate != null ? this.changeDateFormatService.convertDateObjectToString(this.uftContinuityData.value.toDate) : "",
      startDate = this.uftContinuityData.value.fromDate != null ? this.changeDateFormatService.convertDateObjectToString(this.uftContinuityData.value.fromDate) : "",
     endDate =  this.changeDateFormatService.convertDateObjectToString(edate)

    } else{
      this.manualOutsideBtn = false
    }
    var reqParam = [
      { 'key': 'category', 'value': '' },
      { 'key': 'payTo', 'value': "" },
      ...
      { 'key': 'clearDate', 'value': '' },
      { 'key': 'issueDate', 'value': '' },
      { 'key': 'chequeAcct', 'value': '' },
      { 'key': 'planType', 'value': '' },
      { 'key': 'servType', 'value': '' },
      { 'key': 'transCD', 'value': '' },
      { 'key': 'rptCategory', 'value': $('#plan').val() || '' },
      { 'key': 'clearFromDT', 'value': bankDate}, // '10/02/2021', '01/01/2007'
      { 'key': 'clearToDT', 'value': bankDate }, //change format
      { 'key': 'planType', 'value': pType || '' },
      { 'key': 'fileID', 'value': this.fileID }
    ];
    let reportsListColumns = [
      { title: 'Action', data: 'transCD' },
      { title: 'Issue Date', data: 'issueDate' },
      { title: 'Cheque Payee', data: 'payTo' },
      { title: 'Cheque Number', data: 'chequeAcct' },
      { title: 'Cheque Amount', data: 'tranAmt' },
      { title: 'Clear Date', data: 'clearDate' },
    ]
    var tableActions = [
      { 'name': 'edit', 'class': 'table-action-btn edit-ico', 'icon_class': 'fa fa-edit', 'showAction': true },
    ]
    let url = UftApi.getMisMatchedItems;
    if (!$.fn.dataTable.isDataTable('#' + tableId)) {
      this.dataTableService.jqueryDataTable(tableId, url, 'full_numbers', reportsListColumns, this.currentUserService.defaultRecordsGrid, true, true, 'lt', 'irp', 0, [1, 'asc'], '', reqParam, 10, '', "issueDate", 9, [4], [2,3], [1,5], [])
    } else {
      this.dataTableService.jqueryDataTableReload(tableId, url, reqParam)
    }
    this.fileID = ''
  ...
  getlisting() {
    let reverseReqParam = [];
    let columns = [{ title: 'File Id', data: 'fileId' },
    { title: 'File Name', data: 'fileName' },
    { title: 'File Process Date', data: 'fileProcessDt' }
    ]
    var url = UftApi.getQsiBankRecFiles;
    var tableId = "bank-doc-upload-list";
    if (!$.fn.dataTable.isDataTable('#bank-doc-upload-list')) {
      var tableActions = [
        { 'name': 'edit', 'class': 'table-action-btn edit-ico', 'icon_class': 'fa fa-pencil', 'title': 'Edit', 'showAction': '' },
        { 'name': 'delete', 'class': 'table-action-btn del-ico', 'icon_class': 'fa fa-trash', 'title': 'Delete', 'showAction': '' },
      ]
      this.dataTableService.jqueryDataTable(tableId, url, 'full_numbers', columns, this.currentUserService.defaultRecordsGrid, true, true, 'lt', 'irp', undefined, [0, 'asc'], '', reverseReqParam, tableActions, undefined, [2], '', [], [], [], [0], [1, 2])
    } else {
  ...
  onFileChanged(event) {
    this.uploadFormGroup.value.selectedFileName = ""
    this.selectedFile = event.target.files[0];
    var fileSize = event.target.files[0].size;
    this.uploadFormGroup.patchValue({ 'selectedFileName': event.target.files[0].name });
  ...
  */
  onSubmitUploadReportPdf() {
    if (this.allowedValue && !this.fileSizeExceeds) {
      ...
      formData.append('coKey', this.selectedCoKey);
  ...
  */
  ...
  fileUpload() {
    this.hmsDataService.OpenCloseModal('uploadPopUp');
    this.getlisting()
  }

  onSubmitUploadGovEligFileForm() {
    this.showLoader = true;
    this.allowedValue = this.allowedExtensions.includes(this.uploadFormGroup.value.selectedFileName)
    this.showLoader = true
    this.loaderPlaceHolder = "Uploading File....."
    var formData = new FormData()
    let fileName = this.uploadFormGroup.value.selectedFileName.substring(this.uploadFormGroup.value.selectedFileName.lastIndexOf('.') + 1, this.uploadFormGroup.value.selectedFileName.length) || this.uploadFormGroup.value.selectedFileName;
    let fullFileName = this.uploadFormGroup.value.selectedFileName;
    let header = new Headers({ 'Authorization': this.currentUserService.token });
    let options = new RequestOptions({ headers: header });
    formData.append('files', this.selectedFile);
    this.hmsDataService.sendFormData(UftApi.clearCIBCChequeFromFileRecord, formData).subscribe(data => {
      if (data) {
        if (data.code == 200 && data.status == "OK") {
          ...
          this.getBankRecStatusLog(data.result.fileID, data.result.fileName).then(res => {
            if (this.logStatus == "N") {
              let msg = data.hmsMessage.messageShort;
              this.toastrService.success(msg);
              $("#ReportResetFooterButton").trigger('click');
            } else if (this.logStatus == "E") {
              this.fileID = data.result.fileID
              this.sRefNo = data.result.srefNo
              $("#ReportResetFooterButton").trigger('click');
              $("#ListPopUpNewAuto").trigger('click');
            }
          })
          this.getlisting();
        }
          ...
          this.removeExtension()
          this.selectedFileName = ""
          this.toastrService.error(this.translate.instant('uft.toaster.fileContentIsNotValid'));
          this.uploadFormGroup.reset()
        }
      } else {
        this.showLoader = false;
  ...
  reSetData() {
    if ($.fn.dataTable.isDataTable('#bank-history')) {
      var table = $('#bank-history').DataTable();
      table.clear()
      table.destroy();
    }
  }
  ...
  updateManual() {
    if ($('#bank-history').find('.error_field').length !== 0) {
      this.toastrService.error(this.translate.instant('uft.toaster.pleaseEnterValidValues'));
      return false;
    }
    let new_obj = []
    new_obj = this.changedArray.map(function (json) {
      let x = {
        "sbenType": json.servType || '',
        "ntranCd": json.transCD,
        "clrDate": json.clearDate || '',
        "recordNum": json.recordNum || '',
        "acctNumber": json.acctNumber || '',
        "chequeAcct": json.chequeAcct || '',
        "txnStatus": json.txnStatus || '',
        "planType": json.planType || '',
        "chequeAmt": json.tranAmt || '',
        "tranAmt": json.tranAmt || '',
        "businessTypeCd":json.planType || '',
        "chqNumber": json.chequeAcct || '',
        "fileID": json.fileID || ''
      };
      return x;
    });
    let obj = new_obj.filter(function (o) {
      if (o.txnStatus != '') {
        return o
      }
    })
    if (obj.length <= 0) {
      this.toastrService.warning(this.translate.instant('uft.toaster.pleaseSelectValidData'))
      return false
    };
    var reqData = new_obj
    var url = UftApi.manualClearCheque    //new api url
    this.hmsDataService.postApi(url, reqData).subscribe(data => {
      if (data.code == 200 && data.status == "OK") {
        this.toastrService.success(this.translate.instant('uft.toaster.recordUpdatedSuccess'))
      } else if (data.code == 500 && data.status == 'INTERNAL_SERVER_ERROR') {
        this.toastrService.error(this.translate.instant('uft.toaster.somethingWentWrongsPleaseTryAgain'))
      }else if(data.code == 400){
        this.toastrService.error(data.hmsMessage.messageShort || this.translate.instant('uft.toaster.chequeNotFound'))
      }
      this.getList();
    })
  }

  ChangeInputDateFormat(event, idx) {
  ...
  demofileGet(){
    this.getBankRecStatusLog(2172, 'QC_20210217_d5.txt').then(res => {
      if (this.logStatus == "N") {
        let msg = "data.hmsMessage.messageShort";
        this.toastrService.success(msg);
        $("#ReportResetFooterButton").trigger('click');
  ...
  getBankRecStatusLog(fileId, fileName) {
    let promise = new Promise((resolve, reject) => {
      let reqParam = {
        "fileID": fileId,
        "fileName": fileName
      }
      this.hmsDataService.postApi(UftApi.getBankRecErrorLog, reqParam).subscribe(data => {
        if (data.code == 200 && data.status == "OK") {
          this.logStatus = data.result.status
          resolve()
        }
      })
    })
